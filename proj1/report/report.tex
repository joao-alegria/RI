...
implementations.
This class depends on the \texttt{FileParser} as it uses the dictionary 
created by the File Parser.
The \texttt{SimpleTok} \texttt{enizer} contains only one method called 
{\it tokenize()\/}, defined by the parent, that replaces all non-alphabetic 
...
numbers, money amounts and numbers in general, but eliminates any kind of 
...
\begin{quote}
\begin{verbatim}
  def tokenize(self): # of class SimpleTokenizer
    for docID, docContent in self.content.items():
        token = re.sub("[^a-z]+", " ", docContent.lower()).split()
        for t in token:
            if len(t) >= 3:
  ...
  def tokenize(self): # of class ComplexTokenizer
    super().tokenize()
    for docID, docContent in self.content.items():
...
\begin{verbatim}
Simple:
Terms: 'and', 'the', 'with', 'for', 'from', 'patients', 'human', 
'cell', 'cells', 'study'
Total Number of Occurences: 2044099
